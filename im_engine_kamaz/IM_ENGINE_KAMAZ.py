...
def reversCRC(crc):
    temp = 0x0000
    CRC16_HI_RX1 = (crc & 0x00FF) & 0xFF
    CRC16_LOW_RX1 = (crc >> 8) & 0xFF
    temp = CRC16_HI_RX1 * 256 + CRC16_LOW_RX1
    return temp

def CRC16_ver(message, len_message):
    crc = 0xFFFF
    for i in range(0, len_message):
        crc = ((crc << 8) ^ crc16Table[(crc >> 8) ^ message[i]]) & 0xFFFF
    crc = reversCRC(crc)
    return crc
